...
public:
    Vectornav() : Node("mercury_imu") {
        ...
        magCalServer = rclcpp_action::create_server<MagCal>(
            this, "vectornav/mag_cal", std::bind(&Vectornav::handleCalGoal, this, _1, _2),
        ...
        reconnectTimer =
            create_wall_timer(reconnectMS, std::bind(&Vectornav::monitorConnection, this));
    }
...
private:
    ...
    bool vnConnect(const std::string & port, const int baud) {
        ...
        RCLCPP_INFO(
            get_logger(), "Connected to IMU %s at baud %d over %s", mn.c_str(), vs->baudrate(),
            vs->port().c_str());
    ...
    static void asyncPacketReceivedHandler(
        ...
        // auto dumpMsg = mercury_msgs::msg::VnDump();
        // auto vpeMsg = mercury_msgs::msg::VnVpeStatus();
        ...
        try {
            ...
            if (std::isnan(msg.orientation.x) || std::isnan(msg.orientation.y) ||
                ...
                RCLCPP_ERROR(
                    node->get_logger(), "NaN detected in orientation, not publishing IMU message");
                error = true;
            }
            if (abs(msg.orientation.x) > 1 || abs(msg.orientation.y) > 1 ||
                abs(msg.orientation.z) > 1 || abs(msg.orientation.w) > 1) {
                RCLCPP_ERROR(
                    node->get_logger(), "Not publishing IMU message because of bad orientation");
                error = true;
            }
            ...
            if (std::isnan(msg.angular_velocity.x) || std::isnan(msg.angular_velocity.y) ||
                ...
                RCLCPP_ERROR(
                    node->get_logger(),
                    "NaN detected in angular velocity, not publishing IMU message");
                error = true;
            }
            if (abs(msg.angular_velocity.x) > 2 * M_PI || abs(msg.angular_velocity.y) > 2 * M_PI ||
                abs(msg.angular_velocity.z) > 2 * M_PI) {
                RCLCPP_ERROR(
                    node->get_logger(),
                    "Not publishing IMU message because of bad angular velocity");
                error = true;
            }
            ...
            if (std::isnan(msg.linear_acceleration.x) || std::isnan(msg.linear_acceleration.y) ||
                ...
                RCLCPP_ERROR(
                    node->get_logger(),
                    "NaN detected in angular acceleration, not publishing IMU message");
                error = true;
            }
            if (abs(msg.linear_acceleration.x) > 20 || abs(msg.linear_acceleration.y) > 20 ||
                abs(msg.linear_acceleration.z) > 20) {
                RCLCPP_ERROR(
                    node->get_logger(),
                    "Not publishing IMU message because of bad linear acceleration");
                error = true;
            }

            // Fill covariance data
            node->fillCovarianceFromParam("orientation_covariance", msg.orientation_covariance);
            node->fillCovarianceFromParam(
        ...
        if (!error) {
            try {
                ...
                RCLCPP_WARN(
                    node->get_logger(), "IMU failed to publish a succssfully parsed packet");
            }
        }
    }

    static void errorPacketReceivedHandler(
        void * nodeptr, vn::protocol::uart::Packet & errorPacket, size_t packetStartIndex) {
        // Get a handle to the VectorNav class
        auto node = reinterpret_cast<Vectornav *>(nodeptr);
    ...
    void monitorConnection() {
        ...
        try {
            // Try reconnecting
            if (vs && vs->isConnected()) vs->disconnect();
    ...
    rclcpp_action::GoalResponse handleCalGoal(
        ...
        RCLCPP_WARN(
            get_logger(), "Mag cal already in progress or sensor not connected, rejecting request");
    ...
    void executeMagCal(const std::shared_ptr<MagCalGH> goalHandle) {
        ...
        auto result = std::make_shared<mercury_msgs::action::MagCal::Result>();
        ...
        if (hsiMode.hsiMode != vn::protocol::uart::HsiMode::HSIMODE_RUN) {
            RCLCPP_ERROR_STREAM(
                get_logger(), "IMU HSI mode did not return to run after reset! returned mode: "
                                  << hsiMode.hsiMode);
        ...
        while (calSamples < 1000 && !goalHandle->is_canceling()) {
            ...
            auto feedbackMsg = std::make_shared<mercury_msgs::action::MagCal::Feedback>();
    ...
    std::string executeConfigRequest(std::string request) {
        if (!vs->isConnected()) return "IMU not connected";
